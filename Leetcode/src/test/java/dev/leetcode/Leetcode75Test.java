...
public class Leetcode75Test {
  ...
  public void testGcdOfStrings() {
    ...
    tests.forEach(
        test ->
            assertEquals(test.getValue0(), leet.gcdOfStrings(test.getValue1(), test.getValue2())));
  }

  @Test
  @DisplayName("1431. Kids With the Greatest Number of Candies")
  public void testKidsWithCandies() {
    List<Triplet<List<Boolean>, List<Integer>, Integer>> tests =
        List.of(
            Triplet.with(List.of(true, true, true, false, true), List.of(2, 3, 5, 1, 3), 3),
            Triplet.with(List.of(true, false, false, false, false), List.of(4, 2, 1, 1, 2), 1),
            Triplet.with(List.of(false, true, true), List.of(2, 8, 7), 1),
            Triplet.with(List.of(true, false, true), List.of(12, 1, 12), 10));
  }
}
